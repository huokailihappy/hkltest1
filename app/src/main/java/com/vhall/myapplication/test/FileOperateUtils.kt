...
/**
 * @author hkl
 *Date: 2022/3/14 11:41 上午
 */

private const val TAG = "ImageExt"

class FileOperateUtils {
    val myMediaStore: MediaStore = MediaStore()
    val bitmap: Bitmap? = null

    @SuppressLint("ResourceType")
    fun getBitMap(context: Context): Bitmap {
        val resources: Resources = context.resources
        val io: InputStream = resources.openRawResource(R.mipmap.sym_def_app_icon)
        val bd: BitmapDrawable = BitmapDrawable(resources, io)

        val bmp = BitmapFactory.decodeResource(context.resources, R.mipmap.sym_def_app_icon)
        val newb = Bitmap.createBitmap(300, 300, Bitmap.Config.ARGB_8888)
...
fun Bitmap.saveToAlbum(
    ...
    val imageUri = contentResolver.insertMediaImage(fileName, relativePath, outPutFile)
    if (imageUri == null) {
        Log.w(TAG, "insert: error: uri == null")
        return null
    }
    //保存图片
    val outputStream = imageUri.outputStream(contentResolver)
    (outputStream ?: return null).use {
        val format = fileName.getBitmapFormat()
        compress(format, quality, it)
        imageUri.finishPending(context, contentResolver, outPutFile.file)
    }
    return imageUri
